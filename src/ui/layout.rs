...
pub(crate) fn shell_header_lines(app_name: &str, feature_name: &str) -> Vec<Line<'static>> {
    ...
    vec![Line::from(vec![
        ...
        Span::styled(
            "  |  ",
            Style::default().fg(theme::to_ratatui(theme::text_dim())),
        ),
...
pub(crate) fn empty_state_lines(has_entries: bool) -> [Line<'static>; 2] {
    ...
    if has_entries {
        [
            ...
            Line::from(Span::styled(
                "Press c to clear filters or / to change the search query.",
                style,
            )),
        ]
    } else {
        [
            Line::from(Span::styled(
                "Magma is open and waiting for log input.",
                style,
            )),
            Line::from(Span::styled(
                "Pass a file path or pipe newline-delimited JSON into magma.",
                style,
            )),
        ]
    }
...
pub(crate) fn help_lines(
    ...
    vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            format!("Current feature: {feature_name}"),
            Style::default().fg(primary),
        )),
        Line::from(Span::styled(
            "Navigation: Up/Down, j/k, PageUp/PageDown, Home/End",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Mouse: wheel up/down scrolls the current view",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Search: / enters search mode, Enter or Esc exits search mode",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Filter: t d i w e toggle trace/debug/info/warn/error",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Clear: c resets query and level filters",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            format!("Export: x writes filtered view to {export_path}"),
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Live follow: file inputs append new entries while the UI runs",
            Style::default().fg(secondary),
        )),
        Line::from(Span::styled(
            "Visual system: black white red with restrained green/yellow accents",
            Style::default().fg(secondary),
        )),
        Line::from(""),
        Line::from(Span::styled(
